...
docker build -t hinemos-${SERVICE}:${VERSION_TAG} images/${SERVICE}-${HINEMOS_VER} --build-arg FROM=hinemos-${SERVICE}-base:openjdk${JAVA_VER}-${OS} --build-arg MAJOR=${HINMEOS_MAJOR} --build-arg MINOR=${HINEMOS_MINOR}
status=$?
if [ $status -ne 0 ]; then
  exit $status
fi
